...
export default {
  ...
  methods: {
    ...
    async getReview(page) {
      const res = await fetch(`https://shop-review-kh-2022.herokuapp.com/store/all?page=${page}`, {
        method: "GET",
        credentials: "include",
        headers: {
  ...
  watch: {
    ...
    stores(store) {
      ...
      for(let i=0; i<store.length; i++){
        fetch(`https://shop-review-kh-2022.herokuapp.com/review/all?storeId=${store[i]._id}`, {
          method: 'GET',
          credentials: 'include',
        }).then( async (res) => {
          const resData = await res.json();
